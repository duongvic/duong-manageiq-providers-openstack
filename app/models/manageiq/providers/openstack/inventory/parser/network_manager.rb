class ManageIQ::Providers::Openstack::Inventory::Parser::NetworkManager < ManageIQ::Providers::Openstack::Inventory::Parser
  ...
  NET_BLACKLIST = %w(VLAN1014 VLAN1015 ).freeze
  NET_WHITELIST = %w(LeasedLine ).freeze
  ...
  def cloud_networks
    collector.cloud_networks.each do |n|
      next n if NET_BLACKLIST.include?(n["name"])
      ...
      # Parse specific network
      if NET_WHITELIST.any? { |e| network.name.include? e } && network.name.match(/-|-/)
        name_arr = network.name.split(/-|-/)
        user_id = name_arr[0]
        user = User.find_by(:userid => user_id)
        if user
          network.type = "ManageIQ::Providers::Openstack::NetworkManager::CloudNetwork::Private"
          network.external_facing = false
          network.shared = false
          network.evm_owner = user
          network.tenant = user.current_group.tenant
          network.name = "#{name_arr[1]}-#{name_arr[2]}"
        end
      end
  ...
  def cloud_subnets
    collector.cloud_subnets.each do |s|
      ...
      if NET_WHITELIST.any? { |e| subnet.name.include? e } && subnet.name.match(/-|-/)
        name_arr = subnet.name.split(/-|-/)
        user_id = name_arr[0]
        user = User.find_by(:userid => user_id)
        if user
          subnet.evm_owner = user
          subnet.tenant = user.current_group.tenant
          subnet.name = "#{name_arr[1]}-#{name_arr[2]}"
        end
      end
    end
  end
  ...
  def firewall_rule_neutron(rule, security_group)
    ...
    firewall_rule.host_protocol = rule.protocol ? rule.protocol.to_s.upcase.split("-").last : 'Any'
  ...
  def firewall_rule_nova(rule, security_group)
    ...
    firewall_rule.host_protocol = rule.ip_protocol.nil? ? 'Any' : rule.ip_protocol.to_s.upcase
