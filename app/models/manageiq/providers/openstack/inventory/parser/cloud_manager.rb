class ManageIQ::Providers::Openstack::Inventory::Parser::CloudManager < ManageIQ::Providers::Openstack::Inventory::Parser
  ...
  def volume_snapshot_templates
    collector.volume_snapshot_templates.each do |vt|
      ...
      volume_template.location = "N/A"
      volume_template.vendor = "openstack"
      volume_template.publicly_available = false
      snapshot = CloudVolumeSnapshot.find_by(:name => volume_template.name) || CloudVolumeSnapshot.find_by(:ems_ref => volume_template.name)
      unless snapshot.nil?
        owner = User.find(snapshot.evm_owner_id)
        volume_template.evm_owner_id = snapshot.evm_owner_id
        volume_template.miq_group_id = owner.current_group_id
        volume_template.tenant_id = snapshot.tenant_id
      end
    end
  end
  ...
  def make_flavor(f)
    if f.is_public
      ...
      flavor.ephemeral_disk_count = f.ephemeral.nil? ? nil : (f.ephemeral.to_i > 0 ? 1 : 0)
      flavor.cloud_tenants = f.is_public ? collector.tenants.map { |tenant| persister.cloud_tenants.lazy_find(tenant.id) }
  ...
  def miq_templates
    collector.images.each do |i|
      ...
      image.uid_ems = i.id
      image.name = i.name.blank? ? i.id.to_s : i.name.capitalize
      ...
      guest_os = OperatingSystem.normalize_os_name(image.name)
