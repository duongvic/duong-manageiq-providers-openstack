class ManageIQ::Providers::Openstack::NetworkManager < ManageIQ::Providers::NetworkManager
  ...
  def create_cloud_network_queue(userid, options = {})
    task_opts = {
      :action => "creating Network for user #{userid}",
      :userid => userid
    }
    queue_opts = {
  ...
  def create_cloud_subnet_queue(userid, options = {})
    task_opts = {
      :action => "creating Subnet for user #{userid}",
      :userid => userid
    }
    queue_opts = {
      :class_name  => self.class.name,
  ...
  def create_load_balancer_queue(userid, options = {})
    task_opts = {
  ...
  def create_load_balancer(options)
    LoadBalancer.raw_create_load_balancer(self, options)
  end

  def create_network_port(options)
    NetworkPort.raw_create_network_port(self, options)
  end

  def create_network_port_queue(userid, options = {})
    task_opts = {
      :action => "creating network port for user #{userid}",
      :userid => userid
    }
    queue_opts = {
      :class_name  => self.class.name,
      :method_name => 'create_network_port',
      :instance_id => id,
      :priority    => MiqQueue::HIGH_PRIORITY,
      :role        => 'ems_operations',
      :zone        => my_zone,
