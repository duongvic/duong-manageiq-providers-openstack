...
module ManageIQ::Providers::Openstack::CloudManager::ProvisionWorkflow::DialogFieldValidation
  ...
  def validate_private_cloud_network(field, values, dlg, fld, value)
    return nil if allowed_private_cloud_networks.length < 1
    return nil unless get_value(values[:cloud_network_selection_method]) == 'network'
    validate_placement(field, values, dlg, fld, value)
  end

  def validate_public_cloud_network(field, values, dlg, fld, value)
    return nil if allowed_public_cloud_networks.length < 1
    return nil unless get_value(values[:cloud_network_type]) == true
    validate_placement(field, values, dlg, fld, value)
  end
