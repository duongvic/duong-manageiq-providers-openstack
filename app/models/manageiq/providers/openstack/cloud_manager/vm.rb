class ManageIQ::Providers::Openstack::CloudManager::Vm < ManageIQ::Providers::CloudManager::Vm
  ...
  def raw_attach_network_port(options)
    ext_management_system.with_provider_connection(:service => "Compute", :tenant_name => cloud_tenant.name) do |service|
      resp = service.create_os_interface(ems_ref, options)
      subnet = CloudSubnet.find(options[:subnet_id])
      unless subnet.nil?
        interface = resp.body['interfaceAttachment']
        network_port = NetworkPort.new
        network_port.ems_ref = interface['port_id']
        network_port.ext_management_system = subnet.ext_management_system
        network_port.type = "ManageIQ::Providers::Openstack::NetworkManager::NetworkPort"
        network_port.status = interface['port_state']
        network_port.mac_address = interface['mac_addr']
        network_port.device_type = "VmOrTemplate"
        network_port.cloud_tenant = subnet.cloud_tenant
        network_port.tenant = subnet.tenant
        network_port.evm_owner = subnet.evm_owner
        network_port.device_id = options[:vm_id]
        network_port.save!

        network_subnet_port = CloudSubnetNetworkPort.new
        network_subnet_port.cloud_subnet_id = subnet.id
        network_subnet_port.network_port_id = network_port.id
  ...
  def attach_network_port_queue(userid, options={})
    task_opts = {
      :action => "Attach Network Port for user #{userid}",
      :userid => userid
    }
    queue_opts = {
      :class_name  => self.class.name,
      :method_name => 'raw_attach_network_port',
      :instance_id => id,
  ...
  def raw_detach_network_port(options)
    ext_management_system.with_provider_connection(:service => "Compute",
                                                   :tenant_name => cloud_tenant.name) do |service|
      service.delete_os_interface(ems_ref, options[:port_id])
    end
  rescue => e
    _log.error "port=[#{name}], error: #{e}"
    raise MiqException::MiqNetworkPortNotDefinedError, parse_error_message_from_neutron_response(e), e.backtrace
  ...
  def detach_network_port_queue(userid, options={})
    task_opts = {
    ...
    queue_opts = {
      :class_name  => self.class.name,
      :method_name => 'raw_detach_network_port',
      :instance_id => id,
      :priority    => MiqQueue::HIGH_PRIORITY,
      :role        => 'ems_operations',
      :zone        => ext_management_system.my_zone,
